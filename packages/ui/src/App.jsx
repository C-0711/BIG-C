...
import { 
  ...
  Command, Map, Cpu, Database, Clock, Folder, Plug, Send, FileText, Image, Settings
} from "lucide-react";
...
// Navigation items - filtered by module config
const NAV_MAIN = [
  { id: "assistant", Icon: Bot, label: "Assistant", path: "/", gradient: true, module: "assistant" },
  { id: "dashboard", Icon: LayoutDashboard, label: "Dashboard", path: "/dashboard", module: "dashboard" },
];

const NAV_CORE = [
  { id: "skills", Icon: Cpu, label: "Skills", path: "/skills", module: "skills" },
  { id: "knowledge", Icon: Database, label: "Knowledge Base", path: "/knowledge", module: "knowledgeBase" },
  { id: "automation", Icon: Clock, label: "Automation", path: "/automation", module: "automation" },
  { id: "workspaces", Icon: Folder, label: "Workspaces", path: "/workspaces", module: "workspaces" },
];

const NAV_DATA = [
  { id: "integrations", Icon: Plug, label: "Integrations", path: "/integrations", module: "integrations" },
  { id: "publishing", Icon: Send, label: "Publishing", path: "/publishing", module: "publishing" },
  { id: "gallery", Icon: Image, label: "Gallery", path: "/gallery", module: "gallery" },
  { id: "reports", Icon: FileText, label: "Reports", path: "/reports", module: "reports" },
];

const NAV_BUSINESS = [
  { id: "marketing", Icon: Megaphone, label: "Marketing", path: "/marketing", module: "marketing" },
  { id: "product", Icon: Package, label: "Product", path: "/product", module: "product" },
  { id: "analytics", Icon: BarChart3, label: "Analytics", path: "/analytics", module: "analytics" },
  { id: "intelligence", Icon: Network, label: "Intelligence", path: "/intelligence", module: "intelligence" },
  { id: "dpp", Icon: FileCheck, label: "Product Pass", path: "/dpp", module: "productPass" },
  { id: "service", Icon: Headphones, label: "Service", path: "/service", module: "service" },
];

const NAV_SETTINGS = [
  { id: "settings", Icon: Settings, label: "Settings", path: "/settings", module: "settings" },
];
...
function Sidebar() {
  ...
  const filterByModule = (items) => items.filter(item => modules[item.module] !== false);

  const NavSection = ({ items, title }) => {
    const filtered = filterByModule(items);
        ...
        {filtered.map((item) => {
          ...
          return (
            <Link key={item.id} to={item.path} title={!expanded ? item.label : undefined}
              ...
              style={active && !item.gradient ? { backgroundColor: branding.primaryColor } : {}}>
              <Icon size={18} />
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-[#1e2a38] flex flex-col transition-all duration-300 z-50 ${expanded ? "w-56" : "w-16"}`}>
      <div className={`h-14 flex items-center border-b border-[#2a3a4a] ${expanded ? "justify-between px-4" : "justify-center px-2"}`}>
        {expanded && <span className="text-white font-semibold text-sm">{branding.name}</span>}
        {!expanded && <span className="text-white font-bold text-lg">{branding.name?.charAt(0) || "I"}</span>}
      ...
      <nav className="flex-1 py-2 px-2 overflow-y-auto">
        ...
        <div className="mt-auto pt-4 border-t border-[#2a3a4a] mt-4">
          <NavSection items={NAV_SETTINGS} />
        </div>
      </nav>
      ...
      <div className="p-3 border-t border-[#2a3a4a]">
        {expanded ? (
          <div className="flex items-center gap-3 px-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: branding.primaryColor }}>
              <User size={16} />
            </div>
          ...
          <div className="w-8 h-8 mx-auto rounded-full flex items-center justify-center text-white" style={{ backgroundColor: branding.primaryColor }}>
            <User size={16} />
          </div>
        )}
      </div>
...
function DPPLanding() {
  ...
  const branding = useBranding();
  const API = config.api?.baseUrl || `/api`;
    ...
    <div className="min-h-screen bg-[#f5f7fa]">
      <header className="bg-gradient-to-r from-[#1e2a38] to-[#2d3a4a] text-white px-6 py-12">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm mb-4" style={{ backgroundColor: branding.primaryColor }}>
          ...
          <form onSubmit={e => { e.preventDefault(); search(); }} className="flex gap-3 max-w-xl mx-auto">
            ...
            <button type="submit" className="px-8 py-4 rounded-xl font-medium text-white" style={{ backgroundColor: branding.primaryColor }}>
              {loading ? "..." : "Find"}
            </button>
          </form>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {results.map((p, i) => (
              <Link key={`${p.id}-${i}`} to={`/dpp/${p.supplier_pid}`}
                className="bg-white rounded-xl border border-[#e2e8f0] p-5 hover:shadow-lg transition-all"
                style={{ borderColor: "transparent" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = branding.primaryColor}
                onMouseLeave={e => e.currentTarget.style.borderColor = "transparent"}>
                <span className="font-mono font-medium" style={{ color: branding.primaryColor }}>{p.supplier_pid}</span>
